...
TEST_F(DumpTests, DISABLED_dump_index_test) {
  ...
  const std::string mainIndexName = "idx__" + tableName + "__" + t1->nameOfColumn(1);
  const std::string deltaIndexName = "idx_delta__" + tableName + "__" + t1->nameOfColumn(1);
  ...
  i2.addInput(t2);
  i2.addField(1);
  i2.setIndexName("idx__" + tableName + "foo" + "__" + t2->nameOfColumn(1));
  i2.execute();
  access::CreateDeltaIndex di2;
  di2.addInput(t2);
  di2.addField(1);
  di2.setIndexName("idx_delta__" + tableName + "foo" + "__" + t2->nameOfColumn(1));
  di2.execute();
