...
public:
  ...
  hyrise::storage::store_ptr_t linxxxs_ref;
  ...
  void SetUp() {
    ...
    linxxxs_ref = std::dynamic_pointer_cast<storage::Store>(Loader::shortcuts::load("test/lin_xxxs.tbl"));
  }
...
TEST_F(TransactionTests, update_and_delete_same_row) {
  ...
  auto reference = Loader::shortcuts::load("test/reference/txtest_update_and_delete_same_row.tbl");
  auto writeCtx =hyrise::tx::TransactionManager::getInstance().buildContext();
  auto& mod = hyrise::tx::TransactionManager::getInstance()[writeCtx.tid];
  ...
  // create PC to simulate position
  auto pc = PointerCalculator::create(linxxxs, new pos_list_t({4}));
  ...
  // create PC to simulate position
  auto pc2 = PointerCalculator::create(linxxxs, new pos_list_t({before}));
...
TEST_F(TransactionTests, insert_and_delete_same_row) {
  ...
  auto reference = Loader::shortcuts::load("test/lin_xxxs.tbl");
  auto writeCtx =hyrise::tx::TransactionManager::getInstance().buildContext();
  auto& mod = hyrise::tx::TransactionManager::getInstance()[writeCtx.tid];
